...
pub async fn post_save_to_bitvault(
    ...
    let bitvault_url = state.config.bitvault_url.as_deref().ok_or_else(|| {
        (
            ...
            Json(ErrorResponse {
                error: "Bitvault not configured".into(),
            }),
        )
    })?;
    ...
    let resp = builder.send().await.map_err(|e| {
        (
            ...
            Json(ErrorResponse {
                error: format!("Failed to reach Bitvault: {e}"),
            }),
        )
    })?;
    ...
    if !resp.status().is_success() {
        ...
        return Err((
            StatusCode::BAD_GATEWAY,
            Json(ErrorResponse {
                error: format!("Bitvault returned {status}: {text}"),
            }),
        ));
    }
...
pub async fn get_proxy_text(
    ...
    let allowed = Url::parse(bitvault_url).map_err(|_| {
        (
            ...
            Json(ErrorResponse {
                error: "Bitvault URL is misconfigured".into(),
            }),
        )
    })?;
        ...
        || requested.port_or_known_default() != allowed.port_or_known_default()
    {
        return Err((
            ...
            Json(ErrorResponse {
                error: "URL not allowed — must point to the configured Bitvault instance".into(),
            }),
        ));
    }
    ...
    if !resp.status().is_success() {
        ...
        return Err((
            StatusCode::BAD_GATEWAY,
            Json(ErrorResponse {
                error: format!("Bitvault responded with {upstream_status}: {body}"),
            }),
        ));
    }
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|e| {
            (
                StatusCode::BAD_GATEWAY,
                Json(ErrorResponse {
                    error: format!("Failed to read Bitvault response: {e}"),
                }),
            )
        })?;
