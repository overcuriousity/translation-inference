...
pub async fn post_tts(
    ...
    const MAX_TTS_CHUNKS: usize = 256;
    let chunks = split_into_chunks(&req.text, state.config.tts_chunk_size);
    if chunks.len() > MAX_TTS_CHUNKS {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse {
                error: format!(
                    "text is too long to synthesize safely; maximum supported chunk count is {MAX_TTS_CHUNKS}"
                ),
            }),
        ));
    }
    let mut audio_bytes: Vec<u8> = Vec::new();
    let entry = req.target_lang
        .as_deref()
        .and_then(|lang| state.config.tts_voice_map.get(lang))
        .ok_or_else(|| {
            let lang = req.target_lang.as_deref().unwrap_or("(none)");
            (
                StatusCode::BAD_REQUEST,
                Json(ErrorResponse {
                    error: format!(
                        "No TTS voice configured for language '{lang}'. \
                         Add '{lang}:voice@model' to TTS_VOICE_MAP."
                    ),
                }),
            )
        })?;
